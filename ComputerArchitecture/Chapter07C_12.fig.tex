...
\draw[emph data] (REGData.east) node[above right,signal] {Data} -| (xData|-MUXResult.blpin 2) -- (MUXResult.blpin 2) node[above left,value] {rd};
...
{%
    ...
    ResultSrc/01/left/emph control%
}
{
...
\path (REGData) node[value,draw,fill=white,yshift=0cm] {\rotatebox{90}{rd\vphantom{hp}}};
...
\path (LogoCenter) node[draw,minimum width=6cm,minimum height=2.2cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\LARGE\bfseries\rmfamily] (Logo) {S10\;:\;WBMem\\ {}};

\path (LogoCenter) node[align=center,yshift=-0.4cm,text=white,font=\rmfamily\footnotesize] {I/LOAD$\to$S0};

\path (Logo.north west) node[below right,font=\rmfamily\footnotesize,text=white] {S7$\leftarrow$};
...
\path (ControlInfo) node[draw,minimum width=3.0cm,minimum height=5cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\ttfamily] {ResultSrc=01\\ RegWrite};
