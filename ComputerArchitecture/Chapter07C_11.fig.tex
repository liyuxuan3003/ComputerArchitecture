...
% MEM.RD -> REGInstr
\draw[line data] (MEM-RD) -- (xReadData|-MEM-RD);
...
% Instr Split
\foreach \y/\bmax/\bmin/\type/\val in {CU-op/06/00/emph data/,CU-funct3/14/12/emph data/,CU-funct7/31/25/emph data/,REGFILE-A1/19/15/line data/,REGFILE-A2/24/20/line data/,REGFILE-A3/11/07/emph data/xrd,IMMExtend.west/31/07/line data/}
{
...
\draw[emph data] (REGALUOut.east) node[above right,signal] {ALUOut} -- node[below,value] {rd} (MUXResult.blpin 1);
...
% MUXResult.out -> REGPC
\draw[emph data no arrow] (MUXResult.brpin 1) -| (xResult|-yResultWB) node[above left,signal] {Result} node[above left,emph data,yshift=2cm,value] {rd} -- (xResultWD3|-yResultWB);

\draw[line data] (xResultWD3|-yResultWB) -- (xResultAdr|-yResultWB);
...
% Result -> MUXAdr.in2
\draw[line data] (xResultAdr|-yResultWB) node[circ] {} -- (xResultAdr|-MUXAdr.blpin 2) -- (MUXAdr.blpin 2);

% Result -> REGFILE.WD3
\draw[emph data] (xResultWD3|-yResultWB) node[circ] {} -- (xResultWD3|-REGFILE-WD3) -- node[below,value,pos=0.5] {rd} (REGFILE-WD3);
...
\path (REGALUOut) node[value,draw,fill=white,yshift=-0.12cm] {\rotatebox{90}{rd\vphantom{hp}}};
...
\path (LogoCenter) node[draw,minimum width=6cm,minimum height=2.2cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\LARGE\bfseries\rmfamily] (Logo) {S9\;:\;WBALU\\ {}};

\path (LogoCenter) node[align=center,yshift=-0.4cm,text=white,font=\rmfamily\footnotesize] {R/OP$\to$ S0\quad I/OP-IMM$\to$S0\\ J/JAL$\to$S0};

\path (Logo.north west) node[below right,font=\rmfamily\footnotesize,text=white] {S3,S4,S5$\leftarrow$};
...
\path (ControlInfo) node[draw,minimum width=3.0cm,minimum height=5cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\ttfamily] {ResultSrc=00\\ RegWrite};
