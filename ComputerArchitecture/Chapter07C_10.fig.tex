...
% MUXAdr ->[Adr] MEM.A
\draw[emph data] (MUXAdr.brpin 1) -- node[above,signal] {Adr} (MEM-A); 
...
% MEM.RD -> REGInstr
\draw[emph data no arrow] (MEM-RD) -- (xReadData|-MEM-RD);

\draw[line data] (xReadData|-MEM-RD) -- (REGInstr.west|-MEM-RD);
...
\draw[emph data no arrow] (REGRD.east|-yREGRD2)  node[above right,signal] {B} -- (xWriteData|-yREGRD2);
\draw[line data] (xWriteData|-yREGRD2) -- (MUXSrcB.blpin 1);

% REGRD.out2 -> MEM.WD
\draw[emph data] (xWriteData|-yREGRD2) node[circ] {} -- node[left,pos=0.6,signal] {\rotatebox{-90}{WriteData}} (xWriteData|-yWriteDataWB) -- (xWriteDataWB|-yWriteDataWB) |- (MEM-WD) node[above left,value] {rs2};
...
\draw[line data] (MUXSrcA.brpin 1) --  (ALU.blpin 1) node[above left,signal] {SrcA}; 

% MUXSrcB ->[SrcB] ALU.in2
\draw[line data] (MUXSrcB.brpin 1) -| (xSrc|-ALU.blpin 2) -- (ALU.blpin 2) node[above left,signal] {SrcB};
...
\draw[emph data] (REGALUOut.east) node[above right,signal] {ALUOut} -- node[below,value] {imm(rs1)} (MUXResult.blpin 1);
...
% MUXResult.out -> REGPC
\draw[emph data no arrow] (MUXResult.brpin 1) -| (xResult|-yResultWB) node[above left,signal] {Result} node[above left,emph data,yshift=2cm,value] {imm(rs1)} -- (xResultWD3|-yResultWB);

\draw[emph data no arrow] (xResultWD3|-yResultWB) -- (xResultAdr|-yResultWB);

\draw[line data] (xResultAdr|-yResultWB) -- (xREGPCin|-yResultWB) -- node[right,signal] {\rotatebox{90}{PCNext}} (xREGPCin|-REGPC.west) -- (REGPC.west);

% Result -> MUXAdr.in2
\draw[emph data] (xResultAdr|-yResultWB) node[circ] {} -- (xResultAdr|-MUXAdr.blpin 2) node[below right,yshift=-0.5cm,value] {imm(rs1)} -- (MUXAdr.blpin 2);
...
% ALUSrcB -> MUXSrcB
\draw[line control] (CU-ALUSrcB) -| (MUXSrcB.btpin 1) coordinate (xV-ALUSrcB);

% ALUSrcA -> MUXSrcA
\draw[line control] (CU-ALUSrcA) -| (MUXSrcA.btpin 1) coordinate (xV-ALUSrcA);

% ALUControl -> ALU
\draw[line control] (CU-ALUControl) -| (ALU.btpin 1) coordinate (xV-ALUControl);
...
% MemWrite -> MEM.WE
\draw[emph control] (CU-MemWrite) -| (MEM-WE) coordinate (xV-MemWrite);
...
{%
    ...
    AdrSrc/1/right/emph control,
    MemWrite/1/right/emph control,
    IRWrite/0/right/line control,
    RegWrite/0/right/line control,
    ImmSrc/xxx/right/line control,
    ALUSrcA/xx/left/line control,
    ALUSrcB/xx/left/line control,
    ALUControl/xxx/left/line control,
...
\path (REGALUOut) node[value,draw,fill=white,yshift=-0.12cm] {\rotatebox{90}{imm(rs1)\vphantom{hp}}};
...
\path (LogoCenter) node[draw,minimum width=6cm,minimum height=2.2cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\LARGE\bfseries\rmfamily] (Logo) {S8\;:\;MemWrite\\ {}};

\path (LogoCenter) node[align=center,yshift=-0.4cm,text=white,font=\rmfamily\footnotesize] {I/STORE$\to$S0};

\path (Logo.north west) node[below right,font=\rmfamily\footnotesize,text=white] {S2$\leftarrow$};
...
\path (ControlInfo) node[draw,minimum width=3.0cm,minimum height=5cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\ttfamily] {ResultSrc=00\\ AdrSrc=1\\ MemWrite};
