...
\draw[emph data] (MUXPCNext.brpin 1) node[below right,value,align=left] {pc+imm} -- node[above,signal] {PCNext} (REGPC.west);
...
% IMEM Split
\foreach \y/\bmax/\bmin/\type/\val in {CU-op/06/00/emph data/,CU-funct3/14/12/emph data/,CU-funct7/31/25/emph data/,REGFILE-A1/19/15/line data/,REGFILE-A2/24/20/line data/,REGFILE-A3/11/07/emph data/xrd,IMMExtend.west/31/07/emph data/ximm}
{
    \draw[\type] (xInstr|-\y) node[above right,signal] {\scalebox{0.5}{[\bmax :\bmin]}} node[below right,value] {\val} -- (\y);
}

\foreach \y/\type in {CU-funct3/emph data,CU-funct7/emph data,REGFILE-A1/line data,REGFILE-A2/line data,REGFILE-A3/emph data}
{
    \path (xInstr|-\y) node[circ,\type] {};
}

% REGFILE.RD1 ->[SrcA] ALU.in1
\draw[line data] (REGFILE-RD1) -- (xMUXSrcBin1|-REGFILE-RD1) -- (xMUXSrcBin1|-ALU.blpin 1) -- (ALU.blpin 1) node[above left,signal] {SrcA};

% MUXSrcB ->[SrcB] ALU.in2
\draw[line data] (MUXSrcB.brpin 1) -- (ALU.blpin 2) node[above left,signal] {SrcB};
...
\draw[emph data] (xMUXSrcBin2|-IMMExtend.east) -- (ADDPCTarget.blpin 2);
...
% PC -> ADDPCTarget.in1
\draw[emph data] (xPC|-ADDPCTarget.blpin 1) node[circ] {} -- (ADDPCTarget.blpin 1) node[above left,value] {pc};
...
\draw[emph data] (xPCPlus4|-ADDPCPlus4.brpin 2) node[below right] {pc+4} -| (xMUXResultin|-MUXResult.blpin 3) node[below left] {pc+4} -- (MUXResult.blpin 3);

% ADDPCPlus4.out -> MUXPCNext.in1
\draw[line data] (xPCPlus4|-ADDPCPlus4.brpin 2) node[circ] {} |- (xMUXPCNextin1|-yPCPlus4) |- (MUXPCNext.blpin 1);

% ALU.out ->[ALUResult] DMEM.A
\draw[line data] (ALU.brpin 2) node[above right,signal] {ALUResult} -- (xALUResult|-DMEM-A); 
\draw[line data] (xALUResult|-DMEM-A) -- (DMEM-A);

% ALU.out -> MUXResult.in1
\draw[line data] (xALUResult|-DMEM-A) node[circ] {} |- (xMUXResultin|-yALUResult) |- (MUXResult.blpin 1);
...
\draw[emph data] (MUXResult.brpin 1) -| (xResult|-yResultWB) node[above left,signal] {Result} -| (xResultWB|-REGFILE-WD3) node[left,yshift=-0.5cm,value] {pc+4}  -- (REGFILE-WD3);

% ADDPCTarget
\draw[emph data] (ADDPCTarget.brpin 2) node[above right,signal] {PCTarget} -- (xPCTarget|-ADDPCTarget.brpin 2) node[below right,value] {pc+imm} -- (xPCTarget|-yPCTarget) -| (xMUXPCNextin2|-MUXPCNext.blpin 2) -- (MUXPCNext.blpin 2);
...
% PCSrc -> MUXPCNext
\draw[emph control] (CU-PCSrc) -| (MUXPCNext.btpin 1) coordinate (xV-PCSrc);
...
{%
    PCSrc/1/right/emph control,
    RegWrite/1/right/emph control,
    ImmSrc/11/right/emph control,
    ALUSrcB/x/right/line control,
    ALUControl/xxx/right/line control,
    MemWrite/0/right/line control,
    ResultSrc/10/right/emph control%
}
{
...
\path (LogoCenter) node[draw,minimum width=5cm,minimum height=2cm,ultra thick,black,fill=black!50!white,align=center] {\LARGE\bfseries\rmfamily\textcolor{white}{J/JAL}\\ \large\textcolor{white}{jal rd,label}};
