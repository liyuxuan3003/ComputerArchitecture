...
\draw[emph data] (MUXPCNext.brpin 1) node[below right,value,align=left] {imm(pc)} -- node[above,signal] {PCNext} (REGPC.west);
...
% ADDPCTarget
\draw[emph data] (ADDPCTarget.brpin 2) node[above right,signal] {PCTarget} -- (xPCTarget|-ADDPCTarget.brpin 2) node[below right,value] {imm(pc)} -- (xPCTarget|-yPCTarget) -| (xMUXPCNextin2|-MUXPCNext.blpin 2) -- (MUXPCNext.blpin 2);
