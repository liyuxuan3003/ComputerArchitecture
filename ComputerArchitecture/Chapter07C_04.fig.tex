...
% Instr Split
\foreach \y/\bmax/\bmin/\type/\val in {CU-op/06/00/emph data/,CU-funct3/14/12/emph data/,CU-funct7/31/25/emph data/,REGFILE-A1/19/15/line data/,REGFILE-A2/24/20/emph data/xrs2,REGFILE-A3/11/07/line data/,IMMExtend.west/31/07/emph data/ximm}
{
    \draw[\type] (xInstr|-\y) node[above right,signal] {\scalebox{0.5}{[\bmax :\bmin]}} node[below right,value] {\val} -- (\y);
}

\foreach \y/\type in {CU-funct3/emph data,CU-funct7/emph data,REGFILE-A1/line data,REGFILE-A2/emph data,REGFILE-A3/line data}
{
...
% ALU.out -> REGALUOut
\draw[emph data no arrow] (ALU.brpin 2) node[above right,signal] {ALUResult} -- node[below,value] {imm(rs1)} (xALUResult|-ALU.brpin 2); 
...
\path (LogoCenter) node[draw,minimum width=6cm,minimum height=2.2cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\LARGE\bfseries\rmfamily] (Logo) {S2\;:\;MemAdr\\ {}};

\path (LogoCenter) node[align=center,yshift=-0.4cm,text=white,font=\rmfamily\footnotesize] {I/LOAD$\to$S7\quad S/STORE$\to$S8};
...
\path (ControlInfo) node[draw,minimum width=3.0cm,minimum height=5cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\ttfamily] {ALUSrcA=10\\ ALUSrcB=01\\ ALUOp=00};
