...
% ALU.out -> REGALUOut
\draw[emph data no arrow] (ALU.brpin 2) node[above right,signal] {ALUResult} -- node[below,value,align=center] {pc+4\\ (rd)} (xALUResult|-ALU.brpin 2); 
...
\path (LogoCenter) node[draw,minimum width=6cm,minimum height=2.2cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\LARGE\bfseries\rmfamily] (Logo) {S5\;:\;Jump\\ {}};

\path (LogoCenter) node[align=center,yshift=-0.4cm,text=white,font=\rmfamily\footnotesize] {J/JAL$\to$S7};
...
\path (ControlInfo) node[draw,minimum width=3.0cm,minimum height=5cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\ttfamily] {ALUSrcA=01\\ ALUSrcB=10\\ ALUOp=00\\ ResultSrc=00\\ PCUpdate};
