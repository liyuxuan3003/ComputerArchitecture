...
\path (REGPC.west) ++(-0.30,0) coordinate (xREGPCEN);

\path (REGInstr.west) ++(-0.30,0) coordinate (xREGInstrEN);
...
% 4 -> MUXSrcB.in3
\draw[line data] (xMUXSrcBin3|-MUXSrcB.blpin 3) node[left,signal] {4} node[ocirc] {} -- (MUXSrcB.blpin 3);
...
% ALU.out -> REGALUOut
\draw[line data] (ALU.brpin 2) node[above right,signal] {ALUResult} -- (REGALUOut.west);
