...
% ---- X ---- %
\path (MUXPCNext.blpin 1) ++(-0.25,0) coordinate (xMUXPCNextin1);
...
\path (DMEM.west) ++(-0.25,0) coordinate (xALUResult);
...
\draw[line data] (MUXPCNext.brpin 1) -- node[above] {PCNext} (REGPC.west);

% REGPC ->[PC] IMEM.A
\draw[line data] (REGPC.east) -- node[above] {PC} (IMEM-A);

% IMEM -[Instr] |
\draw[line data] (IMEM-RD) -- node[above] {Instr} (xInstr|-IMEM-RD);
...
\draw[line data] (REGFILE-RD1) -|
 (xMUXSrcBin1|-ALU.blpin 1) -- (ALU.blpin 1) node[above left] {SrcA};

% MUXSrcB ->[SrcB] ALU.in2
\draw[line data] (MUXSrcB.brpin 1) -- (ALU.blpin 2) node[above left] {SrcB};
...
\draw[line data] (IMMExtend.east) node[above right] {ImmExt} -- (ADDPCTarget.blpin 2);
...
% 4 -> ADDPCPlus4.in2
\draw[line data] (xPC|-ADDPCPlus4.blpin 2) node[ocirc] {} node[left] {4} -- (ADDPCPlus4.blpin 2);

% ADDPCPlus4.out -> MUXPCNext.in1
\draw[line data] (ADDPCPlus4.brpin 2) node[above right] {PCPlus4} -- (xPCPlus4|-ADDPCPlus4.brpin 2) |- (xMUXPCNextin1|-yPCPlus4) |- (MUXPCNext.blpin 1);

% ALU.out ->[ALUResult] DMEM.A
\draw[line data] (ALU.brpin 2) node[above right] {ALUResult} -- (DMEM-A);
...
\draw[line data] (DMEM-RD) -- node[above] {ReadData} (MUXResult.blpin 2);

% ADDPCPlus4.out -> MUXResult.in3
\draw[line data] (xPCPlus4|-ADDPCPlus4.brpin 2) node[circ] {} -| (xMUXResultin|-MUXResult.blpin 3) -- (MUXResult.blpin 3);

% MUXRestult.out -> REGFILE.WD3
\draw[line data] (MUXResult.brpin 1) -| (xResult|-yResultWB) node[above left] {Result} -| (xResultWB|-REGFILE-WD3) -- (REGFILE-WD3);

% ADDPCTarget
\draw[line data] (ADDPCTarget.brpin 2) node[above right] {PCTarget} -| (xPCTarget|-yPCTarget) -| (xMUXPCNextin2|-MUXPCNext.blpin 2) -- (MUXPCNext.blpin 2);
...
% MEMWrite -> DMEM
\draw[line control] (CU-MemWrite) -| (DMEM-WE);

% ResultSrc -> MUXResult
\draw[line control] (CU-ResultSrc) -| (MUXResult.btpin 1);

% PCSrc -> MUXPCNext
\draw[line control] (CU-PCSrc) -| (MUXPCNext.btpin 1);
