...
\draw[emph data no arrow] (REGFILE-RD2) -- (xMUXSrcBin1|-REGFILE-RD2) node[below left,value] {rs2};
\draw[line data] (xMUXSrcBin1|-REGFILE-RD2) |- (MUXSrcB.blpin 1);

% IMEM.RD2 -> DMEM.WD
\draw[emph data] (xMUXSrcBin1|-REGFILE-RD2) node[circ] {} |- (DMEM-WD) node[below left,value] {rs2};
...
\draw[emph data] (ALU.brpin 2) node[below right,value] {imm(rs1)} node[above right,signal] {ALUResult} -- (DMEM-A);
...
% MEMWrite -> DMEM
\draw[emph control] (CU-MemWrite) -| (DMEM-WE) coordinate (xV-MemWrite);
...
{%
    ...
    ImmSrc/00/right/emph control,
    ALUSrcB/1/right/emph control,
    ALUControl/000/right/emph control,
...
\path (LogoCenter) node[draw,minimum width=5cm,minimum height=2cm,ultra thick,black,fill=black!50!white,align=center] {\LARGE\bfseries\rmfamily\textcolor{white}{S/STORE}\\ \large\textcolor{white}{sw rs2,imm(rs1)}};
