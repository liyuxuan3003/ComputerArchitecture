...
% PC -> MUXSrcA.in1
\draw[line data] (xPC|-yREGInstr1) node[circ] {} |- (xMUXSrcAin1|-yPC) node[below left,value] {pc} |- (MUXSrcA.blpin 1);
...
\draw[emph data] (REGFILE-RD1) -- node[below,value,pos=0.6] {rs1} (REGRD.west|-yREGRD1);

% REGFILE.RD2 -> REGRD.in2
\draw[emph data] (REGFILE-RD2) -- node[below,value,pos=0.6] {rs2} (REGRD.west|-yREGRD2);
...
\draw[emph data] (IMMExtend.east) node[above right,signal] {ImmExt} -- node[below,value] {imm} (xIMMExt|-IMMExtend.east) |- (MUXSrcB.blpin 2);
...
\draw[emph data] (REGInstr.east|-yREGInstr1) node[above right,signal] {PCCurr} -- node[above,value,pos=0.9] {pc} (xMUXSrcAin2|-yREGInstr1) |- (MUXSrcA.blpin 2);
...
\draw[emph data] (MUXSrcB.brpin 1) -| (xSrc|-ALU.blpin 2) -- node[below,value] {imm} (ALU.blpin 2) node[above left,signal] {SrcB};

% ALU.out -> REGALUOut
\draw[emph data no arrow] (ALU.brpin 2) node[above right,signal] {ALUResult} -- node[below,value] {pc+imm} (xALUResult|-ALU.brpin 2); 
...
\path (LogoCenter) node[draw,minimum width=6cm,minimum height=2.2cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\LARGE\bfseries\rmfamily] (Logo) {S1\;:\;Decode\\ {}};

\path (LogoCenter) node[align=center,yshift=-0.4cm,text=white,font=\rmfamily\footnotesize] {I/LOAD$\to$S2\quad S/STORE$\to$S2\\
R/OP$\to$S3\quad I/OP-IMM$\to$S3\\
B/BRANCH$\to$S4\quad J/JAL$\to$S5};

\path (Logo.north west) node[below right,font=\rmfamily\footnotesize,text=white] {S0$\leftarrow$};
...
\path (ControlInfo) node[draw,minimum width=3.0cm,minimum height=5cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\ttfamily] {ALUSrcA=01\\ ALUSrcB=01\\ ALUOp=00};
