...
\foreach \x in {0,1,...,15}
{
    \foreach \y in {0,1,...,7}
    {
        \path ($(xCL|-yCB)+(\fpeval{\x+15}*\ulen,\y*\ulen)$) node[cache bit] (D\y-\x) {} ;  
    } 
}
...
% Set  M6 ~ M7
% Byte M8 ~ M9
\path (O) node[cache block,fit=(M0.south west)(M5.north east)] (MTag) {} ; 
\path (O) node[cache block,fit=(M6.south west)(M7.north east)] (MSet) {} ; 
\path (O) node[cache block,fit=(M8.south west)(M9.north east)] (MByte) {} ; 

% Block of cache way 0
% V    Cn-0  ~ Cn-0
% Tag  Cn-1  ~ Cn-6
% Data Cn-7  ~ Cn-14
\foreach \y in {0,1,...,3}
{
    ...
    \path (O) node[cache block,fit=(C\y-1.south west)(C\y-6.north east)] (C\y Tag) {} ;
    \path (O) node[cache block,fit=(C\y-7.south west)(C\y-14.north east)] (C\y Data) {} ;
}

% Block of cache way 1
% V    Dn-0  ~ Dn-0
% Tag  Dn-1  ~ Dn-6
% Data Dn-7  ~ Dn-14
\foreach \y in {0,1,...,3}
{
    \path (O) node[cache block,fit=(D\y-0.south west)(D\y-0.north east)] (D\y V) {} ;
    \path (O) node[cache block,fit=(D\y-1.south west)(D\y-6.north east)] (D\y Tag) {} ;
    \path (O) node[cache block,fit=(D\y-7.south west)(D\y-14.north east)] (D\y Data) {} ;
}
...
\foreach \y in {0,1,...,3}
{
    \path (O) node[cache block,fit=(D\y-15.south west)(D\y-15.north east)] (D\y U) {} ;
}
...
% Memory.Set -> Cache
\draw[line control,-latex] (MSet.south) node[below right,gray,font=\small] (LBs3) {$\log_{2}S=2$}  |- (C2V.west) node[above left,signal] (SISet) {Set[1:0]}  ;
