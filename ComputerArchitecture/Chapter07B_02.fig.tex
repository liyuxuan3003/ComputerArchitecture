...
\draw[emph data] (DMEM-RD) node[above right,signal] {ReadData} node[below right,value] {[imm(rs1)]} --  (MUXResult.blpin 2);

% MUXRestult.out -> REGFILE.WD3
\draw[emph data] (MUXResult.brpin 1) -| (xResult|-yResultWB) node[above left,signal] {Result} -| (xResultWB|-REGFILE-WD3) node[left,value,yshift=-1cm] {[imm(rs1)]} -- (REGFILE-WD3);
...
{%
    ...
    ResultSrc/01/right/emph control%
}
{
...
\path (LogoCenter) node[draw,minimum width=5cm,minimum height=2cm,ultra thick,black,fill=black!50!white,align=center] {\LARGE\bfseries\rmfamily\textcolor{white}{I/LOAD}\\ \large\textcolor{white}{lw rd,imm(rs1)}};
