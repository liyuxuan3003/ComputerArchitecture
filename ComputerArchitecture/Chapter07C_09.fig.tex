...
\draw[emph data] (xReadData|-MEM-RD) node[circ] {} -- node[right,pos=0.4,signal] {\rotatebox{-90}{ReadData}} (xReadData|-REGData.west) node[above left,yshift=0.25cm,value,align=center] {[imm(rs1)]\\ (rd)} -- (REGData.west);
...
\path (LogoCenter) node[draw,minimum width=6cm,minimum height=2.2cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\LARGE\bfseries\rmfamily] (Logo) {S7\;:\;MemRead\\ {}};

\path (LogoCenter) node[align=center,yshift=-0.4cm,text=white,font=\rmfamily\footnotesize] {I/LOAD$\to$S10};
...
\path (ControlInfo) node[draw,minimum width=3.0cm,minimum height=5cm,ultra thick,black,fill=black!50!white,align=center,text=white,font=\ttfamily] {ResultSrc=00\\ AdrSrc=1};
