...
\draw[emph data] (MUXPCNext.brpin 1) node[below right,value,align=left] {pc+4|\\ pc+imm} -- node[above,signal] {PCNext} (REGPC.west);
...
% IMEM Split
\foreach \y/\bmax/\bmin/\type/\val in {CU-op/06/00/emph data/,CU-funct3/14/12/emph data/,CU-funct7/31/25/emph data/,REGFILE-A1/19/15/emph data/xrs1,REGFILE-A2/24/20/emph data/xrs2,REGFILE-A3/11/07/line data/,IMMExtend.west/31/07/emph data/ximm}
{
    \draw[\type] (xInstr|-\y) node[above right,signal] {\scalebox{0.5}{[\bmax :\bmin]}} node[below right,value] {\val} -- (\y);
}

\foreach \y/\type in {CU-funct3/emph data,CU-funct7/emph data,REGFILE-A1/emph data,REGFILE-A2/emph data,REGFILE-A3/line data}
{
...
\draw[line data] (MUXResult.brpin 1) -| (xResult|-yResultWB) node[above left,signal] {Result} -| (xResultWB|-REGFILE-WD3) -- (REGFILE-WD3);
...
\draw[emph control] (ALU.brpin 1) node[above right,signal] {Zero} -| (xZero|-yZeroCU) node[above,value] {rs1-rs2==0} -| (xZeroCU|-CU-Zero) -- (CU-Zero);

% ---- Control Signal ---- %
\foreach \x/\val/\direc/\type in%
{%
    PCSrc/0|1/right/emph control,
    RegWrite/1/right/emph control,
    ImmSrc/10/right/emph control,
    ALUSrcB/0/right/emph control,
    ALUControl/001/right/emph control,
    MemWrite/0/right/line control,
    ResultSrc/xx/right/line control%
}
{
...
\path (LogoCenter) node[draw,minimum width=5cm,minimum height=2cm,ultra thick,black,fill=black!50!white,align=center] {\LARGE\bfseries\rmfamily\textcolor{white}{B/BRANCH}\\ \large\textcolor{white}{beq rs1,rs2,label}};
