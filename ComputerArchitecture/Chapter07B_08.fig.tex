...
\draw[line control] (xL|-AND.in 1) node[left,signal] {Zero} -- (AND.in 1);
...
\draw[line control] (OR.out) -- (xPCSrc|-OR.out) node[right,signal] {PCSrc};

\draw[ultra thin] ($(xL|-AD.south)+(-2.5,-0.3)$) rectangle ($(xPCSrc|-AND.north)+(1.3,0.3)$); 
